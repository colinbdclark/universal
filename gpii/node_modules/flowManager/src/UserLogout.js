...
(function () {
    ...
    request.onUserLogout = function (requestProxy, token, lifecycleManagerServerDataSource) {
        request.log("A key-out request was received for token", token);
        ...
        lifecycleManagerServerDataSource.set({
            ...
            request.log("Lifecycle manager returned: ", response);
            request.log("Token " + token + " was successfully keyed out.");
    ...
    fluid.defaults("gpii.flowManager.userLogoutSession", {
        ...
        listeners: {
            "{request}.session.events.afterDestroySession": [{
                this: "{request}.res",
                method: "clearCookie",
                args: "{sessionManager}.options.key"
            }, "{that}.afterLogout"]
        },
        invokers: {
            afterLogout: {
